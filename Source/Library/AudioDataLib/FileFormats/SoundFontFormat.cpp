...
/*virtual*/ bool SoundFontFormat::ReadFromStream(ByteStream& inputStream, std::shared_ptr<FileData>& fileData)
{
	...
	std::unique_ptr<SoundFontData> soundFontData(new SoundFontData());
	bool success = false;
	...
	if (smplChunk)
	{
		...
		for (uint32_t i = 0; i < sampleHeaderArray.size(); i++)
		{
			...
			if (!this->ConstructAudioSamples(soundFontData.get(), sampleHeaderArray, audioSampleIDArray, smplChunk, sm24Chunk))
				break;
		}
	}
	...
	fileData.reset(soundFontData.release());
	return true;
}
